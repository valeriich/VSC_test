...
if __name__=="__main__":
    ...
    data = pd.read_csv("/home/valerii/Documents/ml/bentoml-demo/hour.csv", 
                        parse_dates=[1])
    # prepare data
    X = prepare_data(data)
    
    # get separate training sets
    lgbm_train_1 = lgbm.Dataset(X[features1], X['casual'])
    lgbm_train_2 = lgbm.Dataset(X[features2], X['registered'])

    # train models with best parameters
    model_1 = lgbm.train(parameters1, train_set=lgbm_train_1, num_boost_round=117)
    model_2 = lgbm.train(parameters2, train_set=lgbm_train_2, num_boost_round=147)


    # save models to drive
    with open('model_casual.pkl', 'wb') as file:
        pickle.dump(model_1, file, protocol=pickle.HIGHEST_PROTOCOL)

    with open('model_registered.pkl', 'wb') as file:
